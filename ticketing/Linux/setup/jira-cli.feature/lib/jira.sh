...
_jira_jira() {
  local _uri=$1
  shift
  curl $conf_curl_flags -u "$conf_dev_jira_cli_login:$conf_dev_jira_cli_api_token" \
    -H 'Accept: application/json' \
    -H 'Content-Type: application/json' \
    "$@" \
    "$conf_dev_jira_cli_url/rest/api/3/$_uri" | format-json
}
_jira_assign() {
  _jira_get_ticket
...
_jira_create() {
  ...
  _jira_jira "issue" \
    -X POST \
    -d "{\"fields\": {\"project\": {\"id\": \"$conf_dev_jira_cli_project\"}, \"summary\": \"$_summary\", \"description\": \"$_description\", \"issuetype\": {\"id\":\"$_issue_type\"}$jira_custom_fields}}"
}
_jira_comment() {
  _jira_get_ticket
