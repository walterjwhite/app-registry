...
_gsettings_dump() {
	_gsettings_exist "$1" || {
		warn "gsettings do not exist for $1"
		return 0
	}
	mkdir -p $_CONF_APPLICATION_DATA_PATH/$_EXTENSION_NAME
	gsettings list-recursively "$1" | sed -e "s/^$1 //" >$_CONF_APPLICATION_DATA_PATH/$_EXTENSION_NAME/gsettings.conf
	return 0
}
_gsettings_restore() {
	[ ! -e "$_CONF_APPLICATION_DATA_PATH/$_EXTENSION_NAME/gsettings.conf" ] && {
		warn "Unable to restore gsettings for $_CONF_APPLICATION_DATA_PATH/$_EXTENSION_NAME/gsettings.conf [$1]"
		return 0
	}
