...
configure_walterjwhite_conf_restore_post() {
  configure_walterjwhite_xdg_defaults
  configure_walterjwhite_scripts
}
configure_walterjwhite_xdg_defaults() {
  case $_PLATFORM in
  FreeBSD | Linux) ;;
  *)
  ...
  for application in $(ls ~/.config/walterjwhite/xdg-open-defaults); do
    info "Setting defaults for $application"
    while read filetype; do
      info "setting default: $application -> $filetype"
