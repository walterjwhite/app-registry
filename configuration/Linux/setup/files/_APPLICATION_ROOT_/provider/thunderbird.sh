...
Linux | FreeBSD)
  provider_path=${alt_path}$HOME/.thunderbird
  ...
  thunderbird_profile_files="containers.json \
		...
		xulstore.json"
  if [ -e "$provider_path" ] && [ -e ${alt_path}$HOME/.thunderbird/installs.ini ]; then
    for thunderbird_instance_path in $(grep Default ${alt_path}$HOME/.thunderbird/installs.ini | sed -e 's/^.*=//'); do
      ...
      ' ${alt_path}$HOME/.thunderbird/installs.ini >${alt_path}$HOME/.thunderbird/installs.ini.new && mv ${alt_path}$HOME/.thunderbird/installs.ini.new ${alt_path}$HOME/.thunderbird/installs.ini
        continue
      }
...
_configuration_thunderbird_backup_post() {
  local thunderbird_prefs=$(find $provider_data_path -type f -name prefs.js -print -quit)
  [ $? -gt 0 ] && return 0
  ...
  cp "$provider_path"/extensions $provider_data_path 2>/dev/null
  if [ $(wc -l <$provider_data_path/extensions) -eq 0 ]; then
    basename $(find "$provider_path" -type f -path '*/extensions/*.xpi') 2>/dev/null |
      sed -e 's/\.xpi$//' | sort -u >>$provider_data_path/extensions
  fi
}
_configuration_thunderbird_restore_post() {
  [ ! -e $provider_data_path/extensions ] && return
...
_thunderbird_extensions() {
  ...
  for thunderbird_provider_name in $(cat $provider_data_path/extensions 2>/dev/null); do
