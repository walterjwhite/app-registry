...
configure_ssh_clear() {
  warn "Not clearing ssh"
}
configure_ssh_restore_pre() {
  if [ -n "$_BACKUP_SSH" ] && [ -e ~/.ssh ]; then
    warn "_BACKUP_SSH was set, backing up ssh config"
    mv ~/.ssh ~/.ssh.original
  fi
}
configure_ssh_restore_post() {
  mkdir -p ~/.ssh/socket
  find ~/.ssh -type d -exec chmod 700 {} +
  find ~/.ssh -type f -exec chmod 600 {} +
  if [ -n "$_BACKUP_SSH" ]; then
    warn "_BACKUP_SSH was set, restoring ssh/config"
    mv ~/.ssh ~/.ssh.restore
    mv ~/.ssh.original ~/.ssh
