...
_include beep configuration context logging net paths platform wait
: ${_CONF_LOG_HEADER:="##################################################"}
...
configure_walterjwhite_projects_clear() {
  ...
  for project in $(find . -type d -name .git | sed -e 's/\/.git//'); do
    cd $project
    _git-is-clean || warn "$project is dirty"
    git push --all || warn "Unable to push all refs"
    git push --tags || warn "Unable to push all tags"
    cd "$_PLUGIN_CONFIGURATION_PATH"
  done
  cd $opwd
}
configure_walterjwhite_projects_restore() {
  [ ! -e $_CONF_APPLICATION_DATA_PATH/$_EXTENSION_NAME/projects ] && return 1
  local opwd=$PWD
  local project
  while read project; do
    if [ -e ~/.data/$project ]; then
      warn "Data Application: $project already exists"
      continue
    fi
    gclone $project
    cd $opwd
...
configure_walterjwhite_projects_backup() {
  ...
  for project in $(find "$_PLUGIN_CONFIGURATION_PATH" -name .git -type d ! -path '*/app.registry/*' ! -path '*/app-registry/*' |
    ...
    cd $project
    _git_has_uncommitted_work || warn "$project is dirty"
    local branch_name=$(gcb)
    [ -z "$branch_name" ] && branch_name=master
    _git_synced_with_remote $branch_name || warn "$project is not synced with remote"
    cd "$_PLUGIN_CONFIGURATION_PATH"
  done
  cd $opwd
}
