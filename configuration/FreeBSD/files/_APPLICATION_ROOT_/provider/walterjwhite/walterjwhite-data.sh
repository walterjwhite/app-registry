...
configure_walterjwhite_data_clear() {
  local opwd=$PWD
...
configure_walterjwhite_data_restore() {
...
configure_walterjwhite_data_backup() {
  ...
  for data_project in $(find "$_PLUGIN_CONFIGURATION_PATH" -mindepth 2 -maxdepth 2 -type d -name .git ! -path '*/configuration*' | sed -e 's/\/.git//' -e 's/\.\///' | sort -u); do
    ...
    cd $data_project
    _git_has_uncommitted_work || warn "$data_project is dirty"
    local branch_name=$(gcb)
    [ -z "$branch_name" ] && branch_name=master
    _git_synced_with_remote $branch_name || warn "$data_project is not synced with remote"
    cd "$_PLUGIN_CONFIGURATION_PATH"
  done
  cd $opwd
}
