...
_CONFIGURE_PASSWORDSTORE_RESTORE() {
  [ ! -e $_CONF_APPLICATION_DATA_PATH/$_EXTENSION_NAME/git ] && return 1
  rm -rf "$_PLUGIN_CONFIGURATION_PATH"
  gclone $(head -1 $_CONF_APPLICATION_DATA_PATH/$_EXTENSION_NAME/git | awk {'print$2'}) "$_PLUGIN_CONFIGURATION_PATH" >/dev/null 2>&1
}
