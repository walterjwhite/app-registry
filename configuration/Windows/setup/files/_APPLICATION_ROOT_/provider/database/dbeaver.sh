...
Windows)
  provider_path=${alt_path}$APPDATA/DBeaverData
  ;;
Apple)
  provider_path=${alt_path}$HOME/Library/DBeaverData
  ;;
Linux | FreeBSD)
  if [ -n "$XDG_DATA_HOME" ]; then
    provider_path=${alt_path}$XDG_DATA_HOME/DBeaverData
  else
    provider_path=${alt_path}$HOME/.local/share/DBeaverData
  fi
  ;;
esac
...
_configuration_dbeaver_backup_post() {
  find "$provider_data_path" -type f -exec $GNU_SED -i '/SQLEditor.resultSet.ratio=.*/d' {} +
  find "$provider_data_path" -type f -exec $GNU_SED -i '/ui.auto.update.check.time=.*/d' {} +
}
_dbeaver_decrypt() {
