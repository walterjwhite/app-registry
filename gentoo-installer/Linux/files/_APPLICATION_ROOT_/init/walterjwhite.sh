...
_ram() {
	...
	[ $memory_required -ge $system_memory ] && {
		detail "System has insuffient memory to run in memory"
		VOLUME_PATH=root-volume
		return
	}
	detail "Resizing /run volume to match size of images: [$root_imgsize] [$system_memory]"
	mount -o remount,size=$root_imgsize /run
	mkdir -p /run/root-image
	detail 'Copying image into memory'
	rsync -h --info=progress /run/root-volume/root-squashfs.img /run/root-image/root-squashfs.img
	detail 'Copied image into memory'
...
_switch_root() {
	...
	detail "Changes are visible @ /mnt/overlay"
