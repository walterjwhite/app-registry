...
_print_log() {
	...
	[ -n "$tone" ] && _beep "$tone"
	_log_to_file "$level_str" "${_LOG_INDENT}${message}"
	_log_to_console "$color" "${_LOG_INDENT}${message}"
...
_handle_background_notification() {
	...
	if [ -n "$_BACKGROUNDED" ] &&
		[ -n "$_OPTN_INSTALL_BACKGROUND_NOTIFICATION_METHOD}" ]; then
...
_long_running_cmd() {
	...
	local message=""
	if [ -n "$_LOGFILE" ]; then
		message=$(tail -$_CONF_LOG_LONG_RUNNING_CMD_LINES $_LOGFILE | _sanitize_input)
	fi
	_alert "$subject" "$message"
}
_include logging platform context wait beep paths net ide
: ${_CONF_LOG_HEADER:="##################################################"}
...
_REQUIRED_APP_CONF="_CONF_DEV_INTELLIJ_IDE _CONF_DEV_JAVA_IDE_JAVA_HOME"
