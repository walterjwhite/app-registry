...
_init_logging() {
	...
	0)
		[ -z "$_LOGFILE" ] && {
			_LOG_MSG=" - DEBUG" _set_logfile $(_mktemp debug)
		}
		set -x
		;;
	esac
...
_set_logfile() {
	...
	[ "$_LOGFILE" = "$1" ] && print_logfile=0
	mkdir -p $(dirname $1)
	_reset_indent
	[ $_CONF_LOG_CONSOLE -eq 4 ] && {
		_reset_logging
	}
	_LOGFILE=$1
	exec 3>&1 4>&2
	exec >>$_LOGFILE 2>&1
	_CONF_LOG_CONSOLE=4
	[ $print_logfile -eq 1 ] && _log_to_console "$_CONF_LOG_C_WRN" "writing logs to ${_LOGFILE}${_LOG_MSG}"
...
_print_log() {
	...
	[ -z "$_INTERACTIVE" ] && _syslog "$message"
	return 0
}
...
_include logging platform context wait beep paths net virtualization
: ${_CONF_LOG_HEADER:="##################################################"}
...
_RUN_DOCKERFILE_INIT() {
  [ -n "$CONTAINER_PORT" ] && [ -n "$_HOST_PORT" ] && {
    _DETAIL "exposing $CONTAINER_PORT -> $_HOST_PORT"
    DOCKER_ARGS="$DOCKER_ARGS -p$_HOST_PORT:$CONTAINER_PORT"
  }
...
_docker_env() {
  ...
  for env_key in $($_CONF_GNU_GREP -Pv '(^$|^#)' .run/.env | sed -e 's/=.*$//'); do
...
_docker_secrets() {
  ...
  for secret_key in $($_CONF_GNU_GREP -Pv '(^$|^#)' .run/.secrets | sed -e 's/=.*$//'); do
