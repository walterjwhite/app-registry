#!/bin/sh
set -a
_APPLICATION_NAME=web-services
...
_set_logfile() {
	...
	mkdir -p $(dirname $1)
	_reset_indent
	[ -n "$_INTERACTIVE" ] && [ $_CONF_LOG_CONSOLE -eq 2 ] && {
		exec 3>&1 4>&2
		_CONF_LOG_CONSOLE=4
	}
	exec >>$_LOGFILE 2>&1
...
_timeout() {
	...
	$sudo timeout $_OPTIONS $timeout "$@" || {
		...
		$timeout_level "_timeout: $error_message: ${timeout}${timeout_units} - $message ($error_status): $sudo timeout $_OPTIONS $timeout $* ($_USER)"
		return $error_status
	}
}
_waitee_init() {
...
_include logging platform context wait beep paths net web-services
: ${_CONF_LOG_HEADER:="##################################################"}
...
: ${_CONF_RUN_PATH:=/tmp/$_USER/walterjwhite/app}
...
_waitee_init
_waiter
for XML_FILE in $(_FIND_DEFAULT); do
