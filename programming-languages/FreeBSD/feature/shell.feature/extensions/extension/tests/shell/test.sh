...
assert_file_exists() {
	if [ ! -f "$1" ]; then
		_test_failure "assert_file_exists: '$1' does not exist" "$3"
		return 1
	fi
	_test_passed "assert_file_exists: '$1' exists" "$3"
}
assert_file_is_empty() {
	[ -s "$1" ] && {
		_test_failure "assert_file_is_empty: '$1' is not empty" "$3"
		return 1
	}
	_test_passed "assert_file_is_empty: '$1' is empty" "$3"
}
capture_commands() {
	_stop_test_logging
	local output
	...
	printf "%s" "$output" | grep '^+ ' | sed 's/^+ //'
}
_test_failure() {
	test_function_failed_asserts=$(($test_function_failed_asserts + 1))
	_LOGFILE="" _print_log 2 FLL "$_CONF_LOG_C_TEST_FAIL" "$_CONF_LOG_BEEP_TEST_FAILURE" "$1 $2"
	cat "$TEST_TEMP_FILE" >&4
	printf '\n\n' >&4
}
_test_passed() {
	test_function_passed_asserts=$(($test_function_passed_asserts + 1))
	_LOGFILE="" _print_log 2 PSS "$_CONF_LOG_C_TEST_PASS" "$_CONF_LOG_BEEP_TEST_PASS" "$1 $2"
}
_CALL_TEST_FUNCTION() {
	_setup_test_logging
	"$@"
...
_run_tests() {
	...
	_DETAIL "=== Test Summary ==="
	_DETAIL "Total: $test_count, Passed: $pass_count, Failed: $fail_count"
	[ "$fail_count" -eq 0 ]
}
_load_libraries() {
	_INFO "loading libraries"
		...
		for TEST_FUNCTION in $(grep -E '^test_[a-zA-Z0-9_]+ *\(' "$test_file" | cut -d'(' -f1); do
			if [ "$(type "$TEST_FUNCTION" 2>/dev/null | head -1)" = "$TEST_FUNCTION is a function" ]; then
				...
				if (
					...
					_DETAIL "  $TEST_FUNCTION: $TEST_FUNCTION_STATUS | passed: $test_function_passed_asserts | failed: $test_function_failed_asserts"
...
_include
: ${_CONF_LOG_C_TEST_FAIL:=$_CONF_LOG_C_ERR}
: ${_CONF_LOG_C_TEST_PASS:=$_CONF_LOG_C_SCS}
: ${_CONF_LOG_BEEP_TEST_FAIL:=$_CONF_LOG_BEEP_ERR}
: ${_CONF_LOG_BEEP_TEST_PASS:=$_CONF_LOG_BEEP_SCS}
