...
_npm_build_all() {
  for NPM_MODULE in "$@"; do
    ...
    ncu | grep -cqm1 'Run ncu -u to upgrade package.json' && {
      _WARN 'updates detected for npm'
      if [ -z "$_NO_NODEJS_AUTOUPDATE" ]; then
        _WARN "automatically updating npm, to disable pass -no-nodejs-autoupdate or set _NO_NODEJS_AUTOUPDATE"
        ncu -u
        npm install --silent
      else
        _WARN "auto updates are disabled"
      fi
    }
...
_PWD=$PWD
_npm_build_all $(_FIND_DIRS_CONTAINING)
