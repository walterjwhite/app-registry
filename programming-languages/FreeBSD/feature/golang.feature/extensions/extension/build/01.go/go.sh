...
_go_build() {
  ...
  _APP_NAME=$(basename $_COMMAND_DIRECTORY)
  cd $_COMMAND_DIRECTORY
  _INFO "Building $_APP_NAME"
  GO_BUILD_ERRORS=0
...
_go_exec() {
  for _GO_FUNCTION_NAME in "$@"; do
    _GO_FUNCTION=$(printf '%s' $_GO_FUNCTION_NAME | tr '[:lower:]' '[:upper:]')
    _GO_$_GO_FUNCTION || {
      _WARN "$_GO_FUNCTION_NAME produced error(s) - $?"
...
unset GOPATH
if [ "$#" -eq "0" ]; then
  _go_build_all $(_FIND_DIRS_CONTAINING)
else
  _go_build_all "$@"
fi
