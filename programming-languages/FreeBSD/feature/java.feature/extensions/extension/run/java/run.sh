...
_waiter
_javadebug() {
  _DEBUG_ARGS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=$_SUSPEND_JVM,address=$_DEBUG_PORT"
}
_JAVA_NEW_INSTANCE() {
...
_RUN_JAVA_INIT() {
  [ -n "$_JAVA_FRAMEWORK" ] && {
    _require_file /usr/local/walterjwhite/install/extensions/extension/$_ACTION/$_RUN_TYPE/framework/${_JAVA_FRAMEWORK}.sh
    _include /usr/local/walterjwhite/install/extensions/extension/$_ACTION/$_RUN_TYPE/framework/${_JAVA_FRAMEWORK}.sh
  }
  _run_java_locate_application
...
_RUN_JAVA() {
  if [ -n "$_AGENT" ]; then
    _require_file "$_AGENT" _AGENT
    _AGENT_ARGS="${_AGENT_ARGS} -javaagent:$_AGENT"
  fi
  java $_AGENT_ARGS $_DEBUG_ARGS $_JAVA_ARGS -jar $_APPLICATION "$@" &
  _RUN_PID=$!
}
