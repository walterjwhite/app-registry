...
_extension_main() {
  ...
  required_message="$extension_action is not yet implemented by any extensions" file_require "$APP_PLATFORM_PATH/extensions/extension/$extension_action"
  extension_opwd=$PWD
  local _error_count=0
  for extension_run_type_path in $(find $APP_PLATFORM_PATH/extensions/extension/$extension_action -mindepth 1 -maxdepth 1 -type d | sort -V); do
...
_extension_load_type() {
  [ -e $APP_PLATFORM_PATH/extensions/type/$extension_run_type.sh ] && {
    . $APP_PLATFORM_PATH/extensions/type/$extension_run_type.sh
    cd $extension_opwd
  }
}
_extension_run() {
...
: ${conf_dev_checkstyle_jar:=$APP_PLATFORM_PATH/apps/$APPLICATION_NAME/tools/checkstyle/checkstyle.jar}
: ${conf_dev_checkstyle_config:=$APP_PLATFORM_PATH/apps/$APPLICATION_NAME/tools/checkstyle/checks/$conf_dev_checkstyle_version_STYLE_checks.xml}
