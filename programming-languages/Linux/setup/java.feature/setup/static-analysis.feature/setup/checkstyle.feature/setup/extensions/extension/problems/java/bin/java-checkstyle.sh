...
_extension_main() {
  ...
  required_message="$extension_action is not yet implemented by any extensions" file_require "$APP_PLATFORM_EXTENSIONS_PATH/extension/$extension_action"
  extension_opwd=$PWD
  local _error_count=0
  for extension_run_type_path in $(find $APP_PLATFORM_EXTENSIONS_PATH/extension/$extension_action -mindepth 1 -maxdepth 1 -type d | sort -V); do
...
_extension_load_type() {
  [ -e $APP_PLATFORM_EXTENSIONS_PATH/type/$extension_run_type.sh ] && {
    . $APP_PLATFORM_EXTENSIONS_PATH/type/$extension_run_type.sh
    cd $extension_opwd
  }
}
_extension_run() {
...
readonly APPLICATION_NAME=programming-languages
...
_metadata_load
_include_optional . beep context logging net paths platform programming-languages wait
: ${conf_beep_enabled:=0}
