...
_help_print_and_exit() {
  _help_print 'system-wide options' $PLATFORM_ROOT_PATH/install/help/default
  if [ "$APPLICATION_NAME" != "install" ]; then
    _help_print $APPLICATION_NAME $PLATFORM_ROOT_PATH/$APPLICATION_NAME/help/default
    _help_print "$APPLICATION_NAME/$APPLICATION_CMD" $PLATFORM_ROOT_PATH/$APPLICATION_NAME/help/$APPLICATION_CMD
  fi
  exit 0
}
file_require() {
...
_extension_main() {
  ...
  required_message="$extension_action is not yet implemented by any extensions" file_require "$PLATFORM_ROOT_PATH/extensions/extension/$extension_action"
  extension_opwd=$PWD
  local _error_count=0
  for extension_run_type_path in $(find $PLATFORM_ROOT_PATH/extensions/extension/$extension_action -mindepth 1 -maxdepth 1 -type d | sort -V); do
...
_extension_load_type() {
  [ -e $PLATFORM_ROOT_PATH/extensions/type/$extension_run_type.sh ] && {
    . $PLATFORM_ROOT_PATH/extensions/type/$extension_run_type.sh
    cd $extension_opwd
  }
}
_extension_run() {
...
readonly PLATFORM_PATH=$HOME/.local/walterjwhite
...
readonly APP_PLATFORM_PATH=$(printf '%s' $APP_PLATFORM_ROOT/$PLATFORM_ROOT_PATH | tr -s /)
...
readonly APP_PATH=$PLATFORM_PATH/apps/$APPLICATION_NAME
...
: ${conf_dev_checkstyle_jar:=$PLATFORM_ROOT_PATH/apps/$APPLICATION_NAME/tools/checkstyle/checkstyle.jar}
: ${conf_dev_checkstyle_config:=$PLATFORM_ROOT_PATH/apps/$APPLICATION_NAME/tools/checkstyle/checks/$conf_dev_checkstyle_version_STYLE_checks.xml}
