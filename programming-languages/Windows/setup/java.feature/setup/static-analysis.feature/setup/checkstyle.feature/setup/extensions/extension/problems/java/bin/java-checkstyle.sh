...
_extension_main() {
  ...
  required_message="$extension_action is not yet implemented by any extensions" file_require "$EXTENSIONS_PATH/extension/$extension_action"
  extension_opwd=$PWD
  local _error_count=0
  for extension_run_type_path in $(find $EXTENSIONS_PATH/extension/$extension_action -mindepth 1 -maxdepth 1 -type d | sort -V); do
...
_extension_load_type() {
  [ -e $EXTENSIONS_PATH/type/$extension_run_type.sh ] && {
    . $EXTENSIONS_PATH/type/$extension_run_type.sh
    cd $extension_opwd
  }
}
_extension_run() {
