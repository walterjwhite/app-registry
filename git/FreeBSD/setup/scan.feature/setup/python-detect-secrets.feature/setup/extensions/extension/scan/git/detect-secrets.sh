...
_git_in_project_base_path() {
  _path_in_path $GIT_PROJECT_BASE_PATH || _git_in_workspace_path
}
_git_in_user_home() {
  _path_in_path $HOME
}
...
_scan_git_init() {
  ...
  git_project_relative_path=$(printf '%s' $opwd | sed -e "s|$GIT_PROJECT_BASE_PATH/||" -e "s|$GIT_WORKSPACE_BASE_PATH/[^/]*/repos/||")
...
_scan_review() {
  ...
  detect-secrets audit "$report_path" || exit_with_error "detect-secrets audit aborted"
}
