...
_data_app_save() {
  ...
  if [ -n "$git_project_path" ]; then
    cd "$git_project_path" || exit_with_error "unable to enter git_project_path: $git_project_path"
  fi
  git add $@ 2>/dev/null
...
_scan_git_write() {
  cd "$data_app_git_project_path" || exit_with_error "unable to enter data_app_git_project_path: $data_app_git_project_path"
...
_path_in_path() {
  ...
  local test_path=$($GNU_REALPATH -m "$_check_path")
  local current_path=$($GNU_REALPATH -m "$PWD")
  local _escaped_test_path
  _escaped_test_path=$(printf '%s' "$test_path" | $GNU_SED -e 's/[][^$()*+?{}|.\]/\\&/g')
  printf '%s' "$current_path" | $GNU_GREP -qP "^${_escaped_test_path}(\$|/)"
}
...
_scan_new() {
  gitleaks detect --report-path "$report_path" -f json
}
_scan_delta() {
  log_info "commit hash changed, updating report"
  gitleaks detect --log-opts="$scan_last_commit..HEAD" --report-path "$report_path.delta" -f json
...
_report_review_scan_report() {
  ...
  if [ ! -s "$_new_report_file" ]; then
    log_info "$_report_name - report is empty"
    return
  fi
  log_info "$_report_name - findings"
  if jq -e 'length > 0' $_new_report_file >/dev/null; then
