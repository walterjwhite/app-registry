...
[ ! -e .git ] && {
	git init
	warn 'Add a remote to sync secrets'
}
_sed_safe() {
	printf '%s' $1 | sed -e "s/\//\\\\\//g"
}
_include beep context logging net paths platform secrets wait
: ${_CONF_LOG_HEADER:="##################################################"}
...
for _SEARCH_ARG in "$@"; do
  case $_SEARCH_ARG in
  */*)
    error "Search arguments cannot contain '/': $_SEARCH_ARG"
    ;;
  esac
...
[ ! -e $_SECRETS_GPG_PATH ] && error 'No secrets exist'
