...
which pbcopy >/dev/null 2>&1 || _SECRETS_OUTPUT_FUNCTION="stdout"
pbcopy >/dev/null 2 &
1 || _SECRETS_OUTPUT_FUNCTION="stdout"
_SECRETS_GET_STDOUT() {
...
_SECRETS_GET_FIND() {
	...
	[ -z "$matched" ] && error "No secrets found matching: $*"
	[ $matches -ne 1 ] && error "Expecting exactly 1 secret to match, instead found: $matches"
	_SECRET_KEY=$matched
}
