...
_load_media_install_secrets() {
  ...
  while read -r secret_to_copy; do
    ...
    if [ ! -d "$HOME/.password-store/$secret_to_copy" ]; then
      secret_to_copy="$secret_to_copy.gpg"
    fi
    sudo_run mkdir -p "$password_store_path/$secret_directory"
    sudo_run cp -R "$HOME/.password-store/$secret_to_copy" "$password_store_path/$secret_to_copy"
...
_load_media_install_password_store() {
  sudo_run cp $HOME/.password-store/.gpg-id "$2"
  sudo_run cp -R $HOME/.gnupg "$1"
}
