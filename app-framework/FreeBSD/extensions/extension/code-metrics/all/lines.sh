...
_include logging platform context wait beep paths net app-framework
: ${_CONF_LOG_HEADER:="##################################################"}
...
_waitee_init
_waiter
_line_count_get_extensions() {
  name_pattern="*" _FIND_DEFAULT_FILES | sed -e "s/^.*\.//" |
    $_CONF_GNU_GREP -P "^[a-zA-Z][\w]{1,3}$" |
    sort -u | tr '\n' ' '
}
for _FILE_TYPE in $(_line_count_get_extensions); do
  name_pattern="*.$_FILE_TYPE" _FIND_DEFAULT_FILES -exec wc -l {} + |
